using System.ComponentModel;
using System.Globalization;
using System.Runtime.CompilerServices;

namespace OmniRef.App.Services;

public sealed class LocalizationService : INotifyPropertyChanged
{
    private static readonly IReadOnlyDictionary<string, string> English =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ["AppTitle"] = "OmniRef",
            ["NewWorkspace"] = "New",
            ["OpenWorkspace"] = "Open",
            ["Save"] = "Save",
            ["SaveAs"] = "Save as",
            ["AddFiles"] = "Files",
            ["AddFolder"] = "Folder",
            ["AddText"] = "Text",
            ["AddFrame"] = "Frame",
            ["Undo"] = "Undo",
            ["Redo"] = "Redo",
            ["Delete"] = "Delete",
            ["Paste"] = "Paste",
            ["Arrange"] = "Arrange",
            ["Layer"] = "Layer",
            ["ScrollToolbarLeft"] = "Scroll toolbar left",
            ["ScrollToolbarRight"] = "Scroll toolbar right",
            ["ScrollTabsLeft"] = "Scroll tabs left",
            ["ScrollTabsRight"] = "Scroll tabs right",
            ["AlignLeft"] = "Align left",
            ["AlignCenter"] = "Align centers",
            ["AlignRight"] = "Align right",
            ["AlignTop"] = "Align top",
            ["AlignMiddle"] = "Align middles",
            ["AlignBottom"] = "Align bottom",
            ["DistributeHorizontal"] = "Distribute horizontally",
            ["DistributeVertical"] = "Distribute vertically",
            ["BringToFront"] = "Bring to front",
            ["BringForward"] = "Bring forward",
            ["SendBackward"] = "Send backward",
            ["SendToBack"] = "Send to back",
            ["Retry"] = "Retry",
            ["SearchPlaceholder"] = "Search title, path, text or tags",
            ["Search"] = "Search",
            ["SearchOptions"] = "Search options",
            ["ClearSearch"] = "Clear search",
            ["SortByLayer"] = "Sort by layer",
            ["SortByLastAccessed"] = "Sort by last accessed",
            ["SortByRelevance"] = "Sort by relevance",
            ["Properties"] = "Properties",
            ["General"] = "General",
            ["Appearance"] = "Appearance",
            ["Storage"] = "Storage",
            ["Title"] = "Title",
            ["Type"] = "Type",
            ["ImageItem"] = "Image",
            ["FileItem"] = "File",
            ["FolderItem"] = "Folder",
            ["TextItem"] = "Text",
            ["UrlItem"] = "Web link",
            ["FrameItem"] = "Group frame",
            ["Tags"] = "Tags",
            ["Path"] = "Path",
            ["FileName"] = "File name",
            ["FileSize"] = "Size",
            ["TagsHint"] = "Comma-separated tags",
            ["TextStyle"] = "Text style",
            ["FontSize"] = "Size",
            ["TextColor"] = "Text color",
            ["CardColor"] = "Card color",
            ["TextAlignLeft"] = "Align text left",
            ["TextAlignCenter"] = "Center text",
            ["TextAlignRight"] = "Align text right",
            ["Embed"] = "Embed copy",
            ["Export"] = "Export",
            ["UnembedAndRelink"] = "Relink as external",
            ["Relink"] = "Relink",
            ["Reveal"] = "Show in Explorer",
            ["ExternalReference"] = "External reference",
            ["ExternalReferenceDescription"] = "The item stays linked to its source on disk.",
            ["EmbeddedCopy"] = "Embedded copy",
            ["EmbeddedCopyDescription"] = "The file content is stored inside this workspace.",
            ["MissingSourceTitle"] = "Source not found",
            ["AlwaysOnTop"] = "Always on top",
            ["CanvasOnlyMode"] = "Canvas-only mode",
            ["LockCanvas"] = "Lock canvas",
            ["RestoreInterface"] = "Restore interface",
            ["HideTaskbarIcon"] = "Hide taskbar icon",
            ["LockWindow"] = "Lock window",
            ["Theme"] = "Theme",
            ["Language"] = "中",
            ["SwitchLanguage"] = "Switch language",
            ["Exit"] = "Exit",
            ["MinimizeWindow"] = "Minimize",
            ["MaximizeWindow"] = "Maximize or restore",
            ["CloseWindow"] = "Close",
            ["Show"] = "Show OmniRef",
            ["Hide"] = "Hide OmniRef",
            ["NoWorkspace"] = "Drop something worth keeping close.",
            ["NoWorkspaceHint"] = "Create or open a workspace, then drag files, folders and images onto the canvas.",
            ["CreateWorkspace"] = "Create workspace",
            ["Saved"] = "Saved",
            ["Saving"] = "Saving…",
            ["Unsaved"] = "Unsaved",
            ["ReadOnly"] = "Read-only",
            ["SaveFailed"] = "Could not save",
            ["WorkspaceFileMissing"] = "The workspace file was removed or replaced. Automatic saving has stopped to protect your data. Restore the original file or use Save As.",
            ["WorkspaceFileMissingEmbedded"] = "The deleted workspace contains embedded files that cannot be reconstructed from memory. Restore the original workspace file before saving.",
            ["WorkspaceFileReplaced"] = "A different workspace now exists at this path. Use Save As to avoid overwriting it.",
            ["HotkeyConflict"] = "Ctrl+Alt+Space is already used by another application.",
            ["WorkspaceError"] = "Workspace error",
            ["OpenFailed"] = "Could not open the selected workspace.",
            ["MissingSource"] = "The referenced source is missing. Relink it from the Properties panel.",
            ["EmbedTooLarge"] = "Files larger than 512 MB cannot be embedded.",
            ["ReadonlyEmbedded"] = "Embedded files open as read-only copies. Export first to edit.",
            ["FrameDefault"] = "Group",
            ["TextDefault"] = "Type here",
            ["Untitled"] = "Untitled",
            ["CloseTab"] = "Close workspace",
            ["Sidebar"] = "Toggle sidebar",
            ["SidebarShort"] = "Panel",
            ["Grid"] = "Toggle canvas grid",
            ["GridShort"] = "Grid",
            ["GridSnap"] = "Snap to grid (hold Ctrl while dragging to disable)",
            ["GridSnapShort"] = "Snap",
            ["TopShort"] = "Top",
            ["ThemeShort"] = "Theme",
            ["ZoomReset"] = "Reset zoom",
            ["Compact"] = "Compact workspace",
            ["WorkspaceSize"] = "Workspace size: {0}",
            ["WorkspaceStorageSummary"] = "{0} · {1:0}% compressible",
            ["CompactRecommended"] = "Compact workspace (about {0} reclaimable; {1} unused embedded assets)",
            ["CompactionRunningTitle"] = "Compacting workspace",
            ["CompactionRunningMessage"] = "OmniRef is saving pending changes, removing unused embedded assets, and reorganizing the workspace database. You can continue working while this finishes.",
            ["CompactionCompleteTitle"] = "Workspace compacted",
            ["CompactionCompleteMessage"] = "Workspace size: {0} → {1}. Freed {2} ({3:0.#}%).",
            ["CompactionCompleteMessageWithAssets"] = "Workspace size: {0} → {1}. Freed {2} ({3:0.#}%). Unused embedded assets removed: {4}.",
            ["CompactionFailedTitle"] = "Could not compact workspace",
            ["CompactionFailedMessage"] = "Compaction did not finish. {0}",
            ["CloseNotification"] = "Close notification",
            ["ConfirmCloseDirty"] = "This workspace could not be saved. Close it anyway?",
            ["RecoveryNotice"] = "OmniRef did not exit cleanly. Recovery workspaces have been restored.",
            ["DialogOk"] = "OK",
            ["DialogYes"] = "Yes",
            ["DialogNo"] = "No",
            ["StartupFailedTitle"] = "OmniRef startup failed",
            ["RecoveryNoticeTitle"] = "Recovery notice",
            ["UnexpectedErrorTitle"] = "Unexpected error",
            ["ConfirmCloseTitle"] = "Unsaved changes",
            ["ErrorDialogTitle"] = "OmniRef",
            ["SaveDialogTitle"] = "Save OmniRef workspace",
            ["OpenDialogTitle"] = "Open OmniRef workspace",
            ["FilesDialogTitle"] = "Add files",
            ["FolderDialogTitle"] = "Add folder",
            ["ExportDialogTitle"] = "Export embedded file",
            ["UnembedDialogTitle"] = "Choose external source",
            ["RelinkDialogTitle"] = "Relink source"
        };

    private static readonly IReadOnlyDictionary<string, string> Chinese =
        new Dictionary<string, string>(StringComparer.Ordinal)
        {
            ["AppTitle"] = "OmniRef",
            ["NewWorkspace"] = "新建",
            ["OpenWorkspace"] = "打开",
            ["Save"] = "保存",
            ["SaveAs"] = "另存为",
            ["AddFiles"] = "文件",
            ["AddFolder"] = "文件夹",
            ["AddText"] = "文本",
            ["AddFrame"] = "分组框",
            ["Undo"] = "撤销",
            ["Redo"] = "重做",
            ["Delete"] = "删除",
            ["Paste"] = "粘贴",
            ["Arrange"] = "排列",
            ["Layer"] = "图层",
            ["ScrollToolbarLeft"] = "向左滚动工具栏",
            ["ScrollToolbarRight"] = "向右滚动工具栏",
            ["ScrollTabsLeft"] = "向左滚动页签",
            ["ScrollTabsRight"] = "向右滚动页签",
            ["AlignLeft"] = "左对齐",
            ["AlignCenter"] = "水平居中",
            ["AlignRight"] = "右对齐",
            ["AlignTop"] = "顶端对齐",
            ["AlignMiddle"] = "垂直居中",
            ["AlignBottom"] = "底端对齐",
            ["DistributeHorizontal"] = "水平分布",
            ["DistributeVertical"] = "垂直分布",
            ["BringToFront"] = "置于顶层",
            ["BringForward"] = "上移一层",
            ["SendBackward"] = "下移一层",
            ["SendToBack"] = "置于底层",
            ["Retry"] = "重试",
            ["SearchPlaceholder"] = "搜索标题、路径、文本或标签",
            ["Search"] = "搜索",
            ["SearchOptions"] = "搜索选项",
            ["ClearSearch"] = "清空搜索",
            ["SortByLayer"] = "按图层排序",
            ["SortByLastAccessed"] = "按最近访问排序",
            ["SortByRelevance"] = "按匹配度排序",
            ["Properties"] = "属性",
            ["General"] = "常规",
            ["Appearance"] = "外观",
            ["Storage"] = "存储",
            ["Title"] = "标题",
            ["Type"] = "类型",
            ["ImageItem"] = "图片",
            ["FileItem"] = "文件",
            ["FolderItem"] = "文件夹",
            ["TextItem"] = "文本",
            ["UrlItem"] = "网页链接",
            ["FrameItem"] = "分组框",
            ["Tags"] = "标签",
            ["Path"] = "路径",
            ["FileName"] = "文件名",
            ["FileSize"] = "大小",
            ["TagsHint"] = "使用逗号分隔标签",
            ["TextStyle"] = "文本样式",
            ["FontSize"] = "字号",
            ["TextColor"] = "文字颜色",
            ["CardColor"] = "卡片颜色",
            ["TextAlignLeft"] = "文字左对齐",
            ["TextAlignCenter"] = "文字居中",
            ["TextAlignRight"] = "文字右对齐",
            ["Embed"] = "内嵌副本",
            ["Export"] = "导出",
            ["UnembedAndRelink"] = "取消内嵌并重新链接",
            ["Relink"] = "重新定位",
            ["Reveal"] = "在资源管理器中显示",
            ["ExternalReference"] = "外部引用",
            ["ExternalReferenceDescription"] = "此项目始终链接到磁盘上的源文件。",
            ["EmbeddedCopy"] = "内嵌副本",
            ["EmbeddedCopyDescription"] = "文件内容保存在当前工作区中。",
            ["MissingSourceTitle"] = "找不到源文件",
            ["AlwaysOnTop"] = "窗口置顶",
            ["CanvasOnlyMode"] = "无 UI 模式",
            ["LockCanvas"] = "锁定画布",
            ["RestoreInterface"] = "恢复界面",
            ["HideTaskbarIcon"] = "隐藏任务栏图标",
            ["LockWindow"] = "锁定窗口",
            ["Theme"] = "主题",
            ["Language"] = "EN",
            ["SwitchLanguage"] = "切换语言",
            ["Exit"] = "退出",
            ["MinimizeWindow"] = "最小化",
            ["MaximizeWindow"] = "最大化或还原",
            ["CloseWindow"] = "关闭",
            ["Show"] = "显示 OmniRef",
            ["Hide"] = "隐藏 OmniRef",
            ["NoWorkspace"] = "把值得随时查看的内容放在这里。",
            ["NoWorkspaceHint"] = "新建或打开工作区，然后将文件、文件夹和图片拖到画布上。",
            ["CreateWorkspace"] = "新建工作区",
            ["Saved"] = "已保存",
            ["Saving"] = "正在保存…",
            ["Unsaved"] = "未保存",
            ["ReadOnly"] = "只读",
            ["SaveFailed"] = "保存失败",
            ["WorkspaceFileMissing"] = "工作区文件已被删除或替换。为保护当前数据，自动保存已停止。请恢复原文件或使用“另存为”。",
            ["WorkspaceFileMissingEmbedded"] = "已删除的工作区包含无法从内存重建的嵌入文件。请先恢复原工作区文件，再进行保存。",
            ["WorkspaceFileReplaced"] = "该路径下出现了另一个工作区。请使用“另存为”，避免覆盖它。",
            ["HotkeyConflict"] = "Ctrl+Alt+Space 已被其他应用占用。",
            ["WorkspaceError"] = "工作区错误",
            ["OpenFailed"] = "无法打开所选工作区。",
            ["MissingSource"] = "引用的源文件已丢失，请在属性面板中重新定位。",
            ["EmbedTooLarge"] = "超过 512 MB 的文件不能内嵌。",
            ["ReadonlyEmbedded"] = "内嵌文件会以只读副本打开；如需编辑，请先导出。",
            ["FrameDefault"] = "分组",
            ["TextDefault"] = "在这里输入",
            ["Untitled"] = "未命名",
            ["CloseTab"] = "关闭工作区",
            ["Sidebar"] = "显示或隐藏侧栏",
            ["SidebarShort"] = "侧栏",
            ["Grid"] = "显示或隐藏画布网格",
            ["GridShort"] = "网格",
            ["GridSnap"] = "吸附到网格（拖动时按住 Ctrl 可临时关闭）",
            ["GridSnapShort"] = "吸附",
            ["TopShort"] = "置顶",
            ["ThemeShort"] = "主题",
            ["ZoomReset"] = "重置缩放",
            ["Compact"] = "压缩工作区",
            ["WorkspaceSize"] = "工作区占用空间：{0}",
            ["WorkspaceStorageSummary"] = "{0} · 可压缩 {1:0}%",
            ["CompactRecommended"] = "建议压缩工作区（预计可释放约 {0}，含 {1} 个未使用的内嵌资源）",
            ["CompactionRunningTitle"] = "正在压缩工作区",
            ["CompactionRunningMessage"] = "OmniRef 正在保存待处理的修改、移除未使用的内嵌资源并整理工作区数据库。压缩期间可以继续使用工作区。",
            ["CompactionCompleteTitle"] = "工作区压缩完成",
            ["CompactionCompleteMessage"] = "工作区占用空间：{0} → {1}。共释放 {2}（{3:0.#}%）。",
            ["CompactionCompleteMessageWithAssets"] = "工作区占用空间：{0} → {1}。共释放 {2}（{3:0.#}%），并移除了 {4} 个未使用的内嵌资源。",
            ["CompactionFailedTitle"] = "无法压缩工作区",
            ["CompactionFailedMessage"] = "压缩未能完成。{0}",
            ["CloseNotification"] = "关闭通知",
            ["ConfirmCloseDirty"] = "该工作区尚未成功保存，仍要关闭吗？",
            ["RecoveryNotice"] = "OmniRef 上次未正常退出，已恢复临时工作区。",
            ["DialogOk"] = "确定",
            ["DialogYes"] = "是",
            ["DialogNo"] = "否",
            ["StartupFailedTitle"] = "OmniRef 启动失败",
            ["RecoveryNoticeTitle"] = "恢复提示",
            ["UnexpectedErrorTitle"] = "发生错误",
            ["ConfirmCloseTitle"] = "未保存的更改",
            ["ErrorDialogTitle"] = "OmniRef",
            ["SaveDialogTitle"] = "保存 OmniRef 工作区",
            ["OpenDialogTitle"] = "打开 OmniRef 工作区",
            ["FilesDialogTitle"] = "添加文件",
            ["FolderDialogTitle"] = "添加文件夹",
            ["ExportDialogTitle"] = "导出内嵌文件",
            ["UnembedDialogTitle"] = "选择外部源文件",
            ["RelinkDialogTitle"] = "重新定位源文件"
        };

    private IReadOnlyDictionary<string, string> _strings = English;
    private string _configuredLanguage = "auto";

    public event PropertyChangedEventHandler? PropertyChanged;

    public string this[string key] => _strings.TryGetValue(key, out var value) ? value : key;
    public string ConfiguredLanguage => _configuredLanguage;
    public bool IsChinese { get; private set; }

    public void SetLanguage(string language)
    {
        _configuredLanguage = language;
        IsChinese = language.Equals("zh-CN", StringComparison.OrdinalIgnoreCase) ||
                    (language.Equals("auto", StringComparison.OrdinalIgnoreCase) &&
                     CultureInfo.CurrentUICulture.TwoLetterISOLanguageName.Equals(
                         "zh",
                         StringComparison.OrdinalIgnoreCase));
        _strings = IsChinese ? Chinese : English;
        OnPropertyChanged("Item[]");
        OnPropertyChanged(nameof(IsChinese));
        OnPropertyChanged(nameof(ConfiguredLanguage));
    }

    public void Toggle() => SetLanguage(IsChinese ? "en-US" : "zh-CN");

    private void OnPropertyChanged([CallerMemberName] string? propertyName = null) =>
        PropertyChanged?.Invoke(this, new PropertyChangedEventArgs(propertyName));
}
